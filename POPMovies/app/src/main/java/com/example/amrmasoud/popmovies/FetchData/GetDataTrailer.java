 ...
 */
public class GetDataTrailer extends AsyncTask<String,Void,Void> {
    private final Context mContext;

    public GetDataTrailer(Context mContext) {
    ...
    private void GetTrailersDataFromJson(String movieJsonStr, String movieId) throws JSONException {
        ...
        for(int i = 0; i < movieArray.length(); i++) {
            ...
            moviesValues.put(MovieContract.TrailersMovieEntry.COLUMN_TRI_NAME, movieObject.getString(TRAILER_NAME));
            moviesValues.put(MovieContract.TrailersMovieEntry.COLUMN_TRI_LINK, movieObject.getString(TRAILER_KEY));

            cVVector.add(moviesValues);
        }

        // build uri of Trailers and trailers with id
        Uri trailerUri = MovieContract.TrailersMovieEntry.CONTENT_URI;
        Uri trailerWithIdUri = MovieContract.TrailersMovieEntry.buildTrailerUriWithMovieId(Long.valueOf(movieId));
